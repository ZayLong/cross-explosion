...
func _on_move_request(_direction:Vector2i, _node:Node2D):
	...
	virtual_tile_map[new_center_coord].grid_node = _node
	move_response.emit(self.map_to_local(new_center_coord), _node)


func _on_explosion_request(_node:Node2D, _pattern:ExplosionPattern = ExplosionPattern.DIAMOND, _explosion_range:int = 7) -> void:
	...
	for r in _explosion_range if _explosion_range % 2 != 0 else _explosion_range + 1:
		for c in _explosion_range:
			var cell:Vector2i = (center_coord - Vector2i(r,c)) + offset
			if _pattern == ExplosionPattern.PATCH: if (r + c) % 2 == 0: continue ## This makes a cool patchwork kind of pattern!
			if _pattern == ExplosionPattern.CROSS: if cell.x != center_coord.x && cell.y != center_coord.y: continue ## This will make a cross pattern
